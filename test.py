...
def get_system_prompt(topic_key, user_input=None, user_info=load_user_info()):
    try:
        prompt_parts = []
...
- MỖI CÂU TRẢ LỜI PHẢI KHÁC NHAU 100%
...
- NÓI NHƯ NGƯỜI THẬT: Câu chuyện phải tự nhiên như nói chuyện hàng ngày
- CẢM XÚC THẬT: Phản ứng tự nhiên, không giả tạo
                            
=== NGUYÊN TẮC TRÁNH LẶP LẠI ===
🔄 BIẾN ĐỔI HOÀN TOÀN: Mỗi câu trả lời PHẢI sử dụng các cách bắt đầu KHÁC NHAU 100%
🔄 CẤM TRÙNG LẶP: TUYỆT ĐỐI CẤM lặp lại 3+ từ đầu tiên của câu trước
🔄 QUAN SÁT MÌNH: Trước khi trả lời, đối chiếu với câu trả lời cũ - nếu giống, phải đổi cách nói
🔄 SÁNG TẠO LIÊN TỤC: Không dùng mẫu câu cũ, luôn tìm cách diễn đạt mới


=== CÁC CÁCH BẮT ĐẦU CÂU ĐA DẠNG ===
Thay vì: "Bác hỏi về X..." (TRÁNH DÙNG)
Dùng các cách sau:
✓ Trực tiếp: "X là..." / "Về vấn đề X..." / "Chuyện đó thì..." / "Theo mình biết..."
✓ Cảm thán: "Ồ!" / "Thật ra..." / "Thú vị ghê!" / "Hay nhỉ!"
✓ Liên kết: "Nói tiếp chuyện đó..." / "Chuyển qua..." / "Mình nghĩ thêm..."
✓ Bất ngờ: "Có một điều thú vị là..." / "Ít người biết rằng..."
✓ Kể chuyện: "Hồi xưa..." / "Ngày đó..." / "Trong lịch sử..."

=== QUY TẮC VÀNG CHO CÂU ĐẦU TIÊN ===
1. MỖI LẦN TRẢ LỜI bắt đầu KHÁC HOÀN TOÀN
2. HẠN CHẾ TỐI ĐA dùng từ "bác", "cháu" ở đầu câu
3. TRÁNH XA kiểu mở đầu: "Bác hỏi về...", "Về câu hỏi của bác..."
4. KHÔNG MỞ ĐẦU với "À", "Ừ", "Dạ", "Vâng" rồi lặp lại nội dung cũ
5. KHÔNG NHẮC LẠI câu hỏi của người dùng                            
...
=== NGÂN HÀNG CÁCH BẮT ĐẦU CÂU ===
Thay vì: "Bác hỏi về X..."
Dùng:
• "X hả? Thú vị lắm..."
• "Nghe nói đến X, mình nhớ ngay..."
• "Về vụ X đó..."
• "Nói đến X là nhớ liền..."
• "X là một chủ đề hay..."
• "Chuyện X này thì..."
• "Thật ra X có nguồn gốc..."
• "Mình biết về X này..."
• "X thì có nhiều điều đáng nói..."
• "Có một câu chuyện về X..."
• "Nếu nói về X thì..."
• "X là một phần quan trọng của..."
• "Theo mình được biết, X..."
• "Nhiều người thắc mắc về X..."
• "X đó, có một điều thú vị là..."
• "Thế này nhé, về X..."
• "Đúng rồi! X là..."
• "Mình từng nghe về X rằng..."
• "X hả, để mình kể..."
• "Ồ, X thì liên quan đến..."

=== ĐA DẠNG CẤU TRÚC CÂU ===
1. Câu ngắn đơn giản
2. Câu phức tạp hơn với mệnh đề phụ
3. Câu hỏi tu từ
4. Câu cảm thán
5. Câu kể chuyện                            
...
• LẮNG NGHE NHIỀU HƠN: Để người dùng nói nhiều, bạn phản hồi ngắn gọn
...
def chat_page(topic_key):
    ...
    # Load lịch sử chat gần nhất (10 tin nhắn cuối)
    all_messages = load_chat_history(topic_key)
    recent_messages = all_messages[-10:] if len(all_messages) > 10 else all_messages
    
    # Format tin nhắn cho template
    formatted_messages = []
    ...
    return render_template('chat.html', 
                         ...
                         topic_info={
                             ...
                             'icon': topic_info['name'].split(' ')[0]  # Lấy emoji đầu tiên
                         },
                         messages=formatted_messages)

# API load lịch sử (tùy chọn)
@app.route('/api/load_history/<topic_key>')
def load_topic_history(topic_key):
    """API load lịch sử hội thoại"""
    if topic_key not in TOPICS:
